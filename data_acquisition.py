...
def get_metadata_tags(beets_id):
    """ Downloads metadata for a given track id from server
        and extracts bpm and format data 

        Args: beets_id (int or str) : numeric character correspoding to the
                                      track's id on server
...
def beets_save_mp3(beets_id, path_to_save_to=MP3_FOLDER):
    """ downloads file id from server and saves
        to path_to_save_to

        Args: beets_if (int or str) : numeric character correspoding to the
                                      track's id on server
...
def main(id_list):
    """ Downloads a batch of tracks from server.
        If the track is in wav format, directly save,
        if it is mp3, convert and save.
